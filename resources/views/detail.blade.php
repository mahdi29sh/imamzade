...
<script src="{{asset('public/js/jssor.slider-27.4.0.min.js')}}" type="text/javascript"></script>
...
<div id="jssor_1" style="position:relative;margin:0 auto;top:0px;left:0px;width:600px;height:500px;overflow:hidden;visibility:hidden;">
    ...
    <div data-u="slides" style="cursor:default;position:relative;top:0px;left:0px;width:600px;height:500px;overflow:hidden;">
        @foreach($image as $i)
        <div>
            <img data-u="image" src="{{asset("public/images/$i->image")}}" />
        </div>
        @endforeach
    </div>
    <!-- Bullet Navigator -->
...
<table class="table table-striped text-right">
    <thead>
    <tr >
        <th scope="col">{{$imamzade[0]->imamzade_Name}}</th>
        <th scope="col">نام امامزاده</th>
    ...
    <tbody>
    <tr>
        <th scope="row">{{$imamzade[0]->Ancestor}}</th>
        <td>نسب به امام معصوم</td>

    </tr>
    <tr>
        <th scope="row">{{$province[0]->province_Name}} / {{$city[0]->city_Name}}</th>
        <td>استان/شهر</td>

    </tr>
    <tr>
        <th scope="row">{{$imamzade[0]->Address}}</th>
        <td>آدرس</td>

    </tr>
    <tr>
        @if($ziaratname->isEmpty())
            <th scope="row">زیارتنامه ای ثبت نشده است</th>


        @else
            <th scope="row">{{$ziaratname[0]->Ziaratname}}</th>
        @endif
        <td>زیارتنامه</td>
